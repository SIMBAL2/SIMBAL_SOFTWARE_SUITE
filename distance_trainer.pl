...
my $refseq_location = "/usr/local/ProkCompleteRefSeq";	# Change to your own preferred location - this will be used as the default location to check for downloaded genomes
our @rules = ();
our @answers = ();
...
if (($complete || $reference) && ($input_list || $input_acc))
{
	die ("the -complete/-reference flags are mutually exclusive with the -list/-acc flags\n");
}

if ($complete && $reference)
{
	die ("the -complete and -reference flags are mutually exclusive\n");
}
...
if (($complete || $reference) && $local)                                             ## If the user wants to download a second copy of this DB, they need to do so very explicitly becuase they probably shouldn't be
{
	die ("The -complete/-reference and -local flags are mutually exclusive.\nIf you really want to download all RefSeq completed genes locally, specify the local path explicitly with the -db flag");
}
...
sub evaluate_yes                                                                 # sub for calculating a rule - distance rules need to be calculated once per target, but other rules can be done once per genomes
{
    ...
    my $y = shift;
    my $target = shift;
    if (defined($rules[1][$x][$y][3]))                                                    # check to see if a rule is a distance rule - if it is, compute it, don't look for a stored value
    {
...
sub evaluate_attribute_distance                       # determines if any sufficiently high-scoring instance of the given attribute is within range of target sequence
{
	...
	{
	    if (defined($hmm_distance)) {$result = check_distance($target, $hmm_distance);}                    # don't check distance if your rule isn't a distance rule
